...
st.markdown("""
    <h1 style='text-align: center; color: #1DB954;'>🥦 MercaIA</h1>
    <h4 style='text-align: center; color: #AAAAAA;'>Predictor Inteligente de Precios de Alimentos — SIPSA Colombia</h4>
...
col2.metric("R²", "0.70", help="Explica el 70% de la variación en los precios")
col3.metric("MAE", "$1,409 COP/kg", help="Error absoluto medio por kilogramo")
col4.metric("Registros entrenados", "441,304")
...
st.markdown("### 🔍 Consultar Precio Estimado")
st.caption("Selecciona el alimento, la ubicación y la fecha para obtener una estimación del precio en mercados mayoristas.")
...
with col_der:
    ...
    mes      = st.slider("📅 Mes", 1, 12, 3)
    semana   = st.slider("📆 Semana del año", 1, 52, 12)
...
if predecir:
    ...
    st.success(f"El precio estimado de **{alimento}** en **{municipio}, {departamento}** para la semana {semana} del mes {mes} es de **${precio:,.0f} COP/kg** en mercado mayorista.")
    st.caption("⚠️ Estos precios corresponden a mercados mayoristas y pueden diferir de los precios al consumidor final en supermercados o tiendas de barrio.")

    # ============================================================
    # GRÁFICA DE COMPARACIÓN POR MES
    # ============================================================

    st.markdown("### 📈 Variación estimada por semana")
    st.caption(f"Evolución del precio estimado de {alimento} en {municipio} a lo largo de las semanas del mes {mes}.")

    semanas = list(range(1, 15))
    precios_semanas = []
    for s in semanas:
        entrada_s = pd.DataFrame([[alimento_cod, departamento_cod, municipio_cod,
                                   mercado_cod, grupo_cod, mes, s]],
                                 columns=['Alimento_cod', 'Departamento_cod', 'Municipio_cod',
                                          'Mercado_cod', 'Grupo_cod', 'Mes', 'Semana'])
        precios_semanas.append(modelo.predict(entrada_s)[0])
    ...
    # ============================================================
    # GRÁFICA COMPARACIÓN POR MES
    # ============================================================

    st.markdown("### 📊 Comparación de precio por mes")
    st.caption(f"Precio promedio estimado de {alimento} en {municipio} para cada mes del año.")

    meses_nombres = ['Ene', 'Feb', 'Mar', 'Abr', 'May', 'Jun',
                     'Jul', 'Ago', 'Sep', 'Oct', 'Nov', 'Dic']
    precios_meses = []
    for m in range(1, 13):
        entrada_m = pd.DataFrame([[alimento_cod, departamento_cod, municipio_cod,
                                   mercado_cod, grupo_cod, m, 7]],
                                 columns=['Alimento_cod', 'Departamento_cod', 'Municipio_cod',
                                          'Mercado_cod', 'Grupo_cod', 'Mes', 'Semana'])
        precios_meses.append(modelo.predict(entrada_m)[0])

    fig2, ax2 = plt.subplots(figsize=(10, 4))
    fig2.patch.set_facecolor('#0F1923')
    ax2.set_facecolor('#0F1923')
    colores = ['#1DB954' if m != mes - 1 else '#FF4B4B' for m in range(12)]
...
st.markdown("""
    ...
    MercaIA — Proyecto universitario de Inteligencia Artificial<br>
    Datos: SIPSA - DANE | Modelo: Random Forest Regressor<br>
